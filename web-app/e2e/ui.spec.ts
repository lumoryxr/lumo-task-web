...
// ─────────────────────────────────────────────────────────────────────────────
// TC87  Accessibility: OS "reduce motion" disables animations everywhere
//
// Backs the global prefers-reduced-motion reset. With the OS preference set,
// no visible element should still be running a long animation (the breathing
// pet, pulses, floats, shimmers). Scans the page and fails listing any element
// whose computed animation-duration is meaningfully non-zero.
// ─────────────────────────────────────────────────────────────────────────────

test.describe("reduced motion", () => {
  test.use({ reducedMotion: "reduce" });

  test("TC87 – a11y: no long-running animations when OS reduce-motion is set", async ({ page }) => {
    await skipOnboardingAndSignIn(page);
    await mockAPIWithData(page);
    await page.goto("/#/today");
    await expect(page.getByRole("link", { name: /^Today\b/i }).first()).toBeVisible({ timeout: 8_000 });
    // Let any mount animations register, then sample computed styles.
    ...
    const running = await page.evaluate(() => {
      ...
      document.querySelectorAll<HTMLElement>("body *").forEach((el) => {
        ...
        // animation-duration like "3s" / "0.0001s"; flag anything still meaningfully long.
        const secs = cs.animationDuration.split(",").map((d) => parseFloat(d) || 0);
        const maxSecs = Math.max(...secs);
        if (maxSecs > 0.05) {
          offenders.push(`${el.tagName.toLowerCase()}.${String(el.className).slice(0, 30)} ${cs.animationName} ${cs.animationDuration}`);
        }
      });
      return offenders.slice(0, 8);
    });
    expect(running, `animations still running under reduce-motion:\n${running.join("\n")}`).toEqual([]);
  });
});
