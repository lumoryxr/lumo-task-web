...
// route family (`routes/outlook.ts`) had ZERO daily-suite presence — no security,
// robustness, or availability row. It is an OPTIONAL external integration, active
// only when four env vars (LUMO_MS_TENANT_ID/CLIENT_ID/CLIENT_SECRET/USER_EMAIL)
// are set; the daily ephemeral env — and every default self-host that never wires
// up Azure — leaves them ABSENT, so both endpoints have deterministic,
// external-call-free behavior and are cleanly testable here with ZERO network
// egress in the passing state.
//
// Three load-bearing production properties, none previously pinned:
//   • AuthN — both endpoints sit behind `app.use("/*", authMiddleware)`; a
//     missing/garbage token must be 401, never a 5xx (AC1).
//   • Fail-closed / graceful degradation — with the integration unconfigured,
//     `GET /calendar` must short-circuit to a clean 503 OUTLOOK_NOT_CONFIGURED
//     BEFORE any token/Graph fetch. Dropping the `if (!TENANT_ID…)` guard would
//     send a doomed request to `login.microsoftonline.com/undefined/…` and surface
//     as a 502 (or a hang) — a real availability footgun (AC2). The 503 fires
//     AHEAD of the missing-param 400, so passing valid start/end proves it is the
//     config guard (not param validation) that short-circuits — and no outbound
//     call is made in this (correct) state, keeping the suite hermetic.
//   • Config confidentiality — `GET /status` must report `configured:false`
//     truthfully and must not disclose the server mailbox when the integration is
//     off (AC3).
//
// Handlers verified already correct → gap in the tests, not the code (test + docs
// only, no production change). Mutation-tested: dropping the auth middleware
// reddens AC1; dropping the `/calendar` config guard turns AC2 into a 502 (perfect
// specificity); forcing `configured:true` reddens AC3.
// ═══════════════════════════════════════════════════════════════════════════════

describe("DFX · Security/Availability — /v1/outlook route family fail-closed & auth-gated (#411)", () => {
  let olowner: { token: string; id: string };

  before(async () => {
    olowner = await registerUser("outlook-owner");
  });

  test("AC1 · /status and /calendar without a token → 401 UNAUTHORIZED", async () => {
    const status = await api("GET", "/v1/outlook/status");
    assert.equal(status.status, 401, "/outlook/status is behind authMiddleware");
    assert.equal(status.body.error?.code, "UNAUTHORIZED");

    const cal = await api("GET", "/v1/outlook/calendar?start=2026-07-01T00:00:00Z&end=2026-07-02T00:00:00Z");
    assert.equal(cal.status, 401, "/outlook/calendar is behind authMiddleware");
    assert.equal(cal.body.error?.code, "UNAUTHORIZED");
  });

  test("AC1 · garbage bearer token → 401 (never 500)", async () => {
    const status = await api("GET", "/v1/outlook/status", { token: "not.a.real.jwt" });
    assert.equal(status.status, 401, "a malformed bearer must be a clean 401, not a 5xx");
    assert.equal(status.body.error?.code, "UNAUTHORIZED");

    const cal = await api("GET", "/v1/outlook/calendar?start=2026-07-01T00:00:00Z&end=2026-07-02T00:00:00Z", {
      token: "not.a.real.jwt",
    });
    assert.equal(cal.status, 401);
    assert.equal(cal.body.error?.code, "UNAUTHORIZED");
  });

  test("AC2 · unconfigured integration → GET /calendar degrades to 503 OUTLOOK_NOT_CONFIGURED before any outbound call", async () => {
    // Valid start/end supplied on purpose: the config guard runs AHEAD of the
    // missing-param 400, so a 503 here proves it is the fail-closed guard that
    // fired (not param validation) — and no token/Graph fetch is attempted, so the
    // suite makes zero outbound network calls in this (correct) state.
    const { status, body } = await api(
      "GET",
      "/v1/outlook/calendar?start=2026-07-01T00:00:00Z&end=2026-07-02T00:00:00Z",
      { token: olowner.token },
    );
    assert.equal(status, 503, "an unconfigured optional integration must fail closed with 503, never a 5xx crash");
    assert.equal(body.error?.code, "OUTLOOK_NOT_CONFIGURED");
  });

  test("AC3 · GET /status reports the integration off truthfully and does not leak the server mailbox", async () => {
    const { status, body } = await api("GET", "/v1/outlook/status", { token: olowner.token });
    assert.equal(status, 200);
    assert.equal(body.configured, false, "with LUMO_MS_* absent, status must report the integration is off");
    assert.ok(
      body.userEmail == null,
      "an unconfigured integration must not disclose the server mailbox (userEmail is null/absent)",
    );
  });
});
