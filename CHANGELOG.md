...
### Changed
- refactor(api)!: **the API surface is now generated from a single route registry**, closing a design↔runtime gap that had grown to 54 endpoints. The list of endpoints previously lived in four places that disagreed: a 693-line hand-written OpenAPI document in `backend/src/routes/docs.ts` (22 endpoints), a stale `docs/openapi.yaml` (17), `docs/openapi.generated.json` (component schemas, **no paths at all**), and the Hono router (84). Nothing detected the gap, so the published docs described under a third of the live API. All of it collapses into `packages/contracts/src/registry.ts` (`API_ROUTES`), which declares every endpoint — method, path, auth mode, request and response schemas — referencing the contract Zod schemas. `buildOpenApiDocument()` generates the complete OpenAPI 3.0.3 document from it; the backend serves that at `GET /docs/openapi.json` and `gen:openapi` writes the identical bytes to `docs/api/openapi.json`. `routes/docs.ts` drops from 693 lines to 29 with no schema of its own, and the two stale specs are deleted. **Enforced, not advisory:** a parity test diffs the registry against `app.routes` and fails in *both* directions — a mounted route with no registry entry is undocumented, and a registry entry with no route means the docs promise a 404 — plus a new `make openapi-check` (and CI job) that regenerates the document and fails if the committed one is stale
- refactor(contracts): **migrated every remaining domain into `@lumo/contracts`** — auth, projects, habits, countdowns, settings, focus/completed, storage, and the AI request bodies. This removes the last inline `z.object()` JSON-body validator from the route layer, so credential rules (password strength, username grammar, reserved names), the goal/KPI grammar, habit frequency vocabulary and AI context bounds are now visible to the frontend and to the published docs instead of being buried in a route file. The contract-first standards scan widens from a two-file allow-list (`tasks.ts`, `people.ts`) to a **repo-wide deny-by-default** check — every domain is migrated, so there is nothing left to exempt — and gains guards that `routes/docs.ts` declares no paths and that no response schema exposes a secret-shaped field

### Fixed
- fix(reliability): **`GET /ready` is now bounded** and answers instead of hanging. It ran an unbounded `SELECT 1`; an unreachable database does not refuse the connection, it hangs, so the probe hung with it until the platform's request timeout fired. A load balancer then observes a timeout rather than a clean 503 — a weaker, slower signal that keeps routing real user traffic to a dead instance meanwhile. The probe now races a 2-second deadline (`READINESS_TIMEOUT_MS`) and reports `503 { ok: false, db: "down" }` if the deadline wins. `GET /health` stays shallow by design
- fix(reliability): **the VPS deployment's container logs are now size-bounded.** `docker-compose.prod.yml` had no `logging:` block, so the app's per-request JSON access logs and Caddy's TLS logs both fell into Docker's default `json-file` driver with **no size cap**. At marketing-time traffic, that driver fills the host disk; once full, SQLite refuses every write and the whole app goes 500 — **and** Docker itself starts failing because its driver can't allocate. Now both `app` and `caddy` declare `max-size: 10m`, `max-file: 3` (a 30m cap per service, rotated by Docker) — enough headroom to investigate, low enough to never threaten the disk. The `lumo-logs` named volume is left in place with a note (it would otherwise be a silent behavioural change if `LUMO_LOG_FILE` is re-enabled later). Doc: `docs/ops/vps-deployment.md` notes the caddy log filter `lumo_access`. Closes `docs/business/commercial-plan-post-launch.md` §3.1
- fix(reliability): **on-box backups are now actually wired up.** `bootstrap.sh` didn't install `sqlite3`, `gzip`, or `cron`, and didn't install a cron entry — so the existing `deploy/vps/backup.sh` (a `sqlite3 .backup` + gzip + 14-day retention + opt-in rclone) sat unused on every box provisioned via bootstrap. The DB-backup.yml workflow it was meant to complement needs `TURSO_DATABASE_URL`, which VPS-mode doesn't set, so it always skipped silently — the on-box path was the only real one, and it never ran. `bootstrap.sh` now (a) installs the missing tooling via apt/dnf/yum (whichever the box has), with **hard fail** (exit 1, not warn) if none of those are available or any tool is still missing after install, (b) installs an idempotent nightly 03:15 cron entry writing to `/var/log/lumo/backup.log` with **fail-fast + post-install verify** (`crontab -l | grep` confirms the line is live), rewriting its own previous entry on re-runs, and (c) ensures `/var/log/lumo` exists up-front so the cron entry can't fail on a missing parent. The finish-setup message points the operator at `crontab -l`, the `RCLONE_REMOTE` env var for off-box durability, and reminds that **an unverified backup is not a backup** (run `backup.sh` and open the `.db.gz` in a throwaway container). Closes `docs/business/commercial-plan-post-launch.md` §3.2 (config-side). The restore drill itself is a one-time operator action, not scriptable into bootstrap.
- fix(observability): **process crashes are written to the structured log** before the process exits. Every deliberate log line goes through `lib/logger.ts` (JSON per line, credential keys redacted, optional `LUMO_LOG_FILE` sink) — but an unhandled rejection or uncaught exception did none of that, because Node's default prints a raw stack trace to stderr and exits. The single most important event in the life of the process produced the one record an aggregator could not index, redaction never touched, and the file sink never received. Crashes now emit `{"level":"error","fatal":true,"signal":…,"reason":…,"stack":…}` and still exit non-zero so the platform restarts. Grep production logs for `"fatal":true`
- fix(api): **rate-limited responses state their own deadline.** A 429 carried no `Retry-After`, so clients guessed — and ours guessed badly: `web-app`'s client retried on the same fixed 300ms/600ms backoff it uses for a network blip, so against the 60-second auth window it spent both retries inside a second, both certain to fail, then surfaced an error the user need never have seen. Every rate-limited response now carries `RateLimit-Limit` / `RateLimit-Remaining` / `RateLimit-Reset`, a rejected one carries `Retry-After` (rounded up, so it never names a deadline that has not passed), and the client waits the stated delay — capped at 30s so a hostile or misconfigured proxy cannot park a request behind a spinner
- fix(perf): **the rate limiter reclaims expired entries again.** Pruning ran when `hits.size % 100 === 0`, which only holds while the map is growing — a limiter that reached a steady population of distinct keys never pruned again and retained every key it had ever seen. On an endpoint keyed by client IP that is an attacker-controlled memory footprint. Replaced with a time-based sweep
- fix(migrations): the `users` table-rebuild migration no longer fails at boot with `SQLITE_CONSTRAINT: FOREIGN KEY constraint failed` (`statementIndex 3` — the rebuild's `DROP TABLE users`). Reported from the Windows desktop build: the backend never started (`Backend did not start in time`) because migrations aborted. Root cause: `DROP TABLE users` performs an **implicit row-delete** of the users it removes, and **libsql enables `PRAGMA foreign_keys` by default** (unlike better-sqlite3, which the old comment assumed) — so on any real database where child tables (`tasks`, `settings`, `people`, …) already reference `users(id)`, that delete violated the FK and bricked every boot. Fresh/empty databases (and the prior test, which seeded a user with no child rows) slipped through, which is why it only surfaced on installed desktop copies with data. Fix: suspend FK enforcement (`PRAGMA foreign_keys = OFF`) around the rebuild `batch()` and restore it in a `finally` (`… = ON`) — the toggle lives outside the transaction because a `PRAGMA foreign_keys` inside `BEGIN…COMMIT` is a silent no-op. Child rows are preserved; enforcement is never left OFF. Regression pinned in `backend/src/test/api/migrate-rebuild.test.ts` (seeds a `tasks` row referencing the user, asserts the rebuild completes, data survives, and FK is re-enabled)
...
### Added
- feat(feedback): **in-app feedback with admin review** (#473 follow-up) — any signed-in user can send feedback (bug / idea / question / other) from the About & Support tab and see the team's status + reply on their own list, without leaving the app; the public GitHub Issues link stays. Contract-first (`@lumo/contracts/feedback`): `POST`/`GET /v1/feedback` (rate-limited submit, own list + an `isAdmin` capability flag) and admin-only `GET`/`PATCH /v1/admin/feedback` (queue + status/reply). Admin is an **email allow-list** (`LUMO_ADMIN_EMAILS`, comma-separated, case-insensitive) rather than a DB role — power stays out of the database (a DB leak grants nobody admin) and suits the single-tenant model (ADR-0004); the frontend reveals the admin triage panel only when the backend reports `isAdmin`. New server-side `feedback` table (not syncable) added to `USER_SCOPED_TABLES` so account deletion purges it (GDPR-erasure guard). New `FORBIDDEN` (403) error code; OpenAPI component schemas generated. Four test layers: contract (13), backend API (12: create/list/conformance/isAdmin/reply round-trip/404), backend security (5: 401/403 gate, empty allow-list grants nobody, case-insensitive match), frontend component (6)
...
- feat(logging): optional **local file sink** for the backend logger (`LUMO_LOG_FILE`) — self-hosted / VPS deployments can keep JSON-line logs on disk (in addition to stdout/stderr) with no hosted aggregator or Sentry. Goes through the single logger seam so the level filter and secret redaction apply first; a write failure disables the sink with a one-time diagnostic and never throws into a request. 5 new logger tests; `docs/ops/logging.md` updated
- chore(marketing): consolidated to a **single canonical marketing site** — the Astro `website/` (published to `gh-pages`); removed the superseded static `landing/` and its `deploy-landing.yml`. Carried the truthful-copy reconciliation to `website/` (FAQ "end-to-end / private" wording implied E2E encryption; reworded to state data is stored on our servers, used only to run the account, no third-party ad trackers, open source)
- chore(commercial): **commercialization-readiness prep** (free-beta phase; docs + CI only, no app-logic change) — CI now runs `npm audit` on `packages/contracts` (it ships in the production backend bundle) + a matching Dependabot entry; a committed, **opt-in** scheduled DB-backup workflow (`db-backup.yml`) that skips cleanly until `TURSO_DATABASE_URL` is set; new ops runbooks (`docs/ops/reliability-monitoring.md`, `docs/ops/runbook.md`) and a `docs/security/pre-launch-security-review.md`; `docs/architecture/overview.md` corrected to the real stack (`@libsql/client` + raw parameterized SQL, no Drizzle/Postgres, no service layer); and the landing privacy copy made **truthful across desktop vs hosted** (desktop-only "offline/no-account/nothing-sent" claims scoped to desktop; "zero telemetry" → "no third-party ad tracking"). Tracked in epic #476
...
- docs: `docs/business/launch-checklist.md` (P0/P1/P2 public-beta readiness) and `docs/business/go-to-market.md` (promotion plan)
...
- ci: GitLab CI/CD migrated from GitHub Actions (`.gitlab-ci.yml`) — 13-job gate, tag Release, manual Windows packaging; `.gitlab/` issue/MR templates, root `CODEOWNERS`, `docs/process/engineering-process.md`
...
- docs: Industry-standard bilingual documentation — README.md (EN), README.zh.md (ZH), updated docs/architecture/overview.md, CHANGELOG.md, new docs/product/roadmap.md (Issue #129)
- Engineering standards documentation (CONTRIBUTING.md, docs/architecture/overview.md)
