...
- [ ] **Micro-interactions** — purposeful transitions, all gated on `reduced_motion`: ~~task-complete check animation~~ (PR #xxx — filled-circle pop + short dwell before the row leaves; dwell skipped under in-product/OS reduced motion), quadrant drag settle, pet reaction on streak/level-up, ~~toast enter/exit easing~~ (already shipped — `ToastStack` translateX+opacity with `--ease-enter`), page/route cross-fade (RouteFallback). Prefer CSS transitions + a shared `motion` token over per-component magic numbers. *Remaining: quadrant drag settle, pet streak/level-up reaction.*
