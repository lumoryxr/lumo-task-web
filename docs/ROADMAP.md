...
- [ ] **i18n completeness** — sweep remaining pages for hardcoded/untranslated EN or ZH strings. *(progress: `MatrixPage` copy localized — PR #101; static key-presence guard #108 + render-layer Playwright guard #109 now catch bare keys; CJK-attribute guard #134; **8 hardcoded English `aria-label`s localized — PR #141, merged 2026-06-27**.)* **Known offenders to route through `useT()`:** the `locale === "zh" ? … : …` inline ternaries in `ShareCard`/`WrappedCard` button labels (`生成中…`/`Exporting…` etc.), and any remaining literals in `FocusPage`/`SettingsPage`. Add a lint/guard for raw CJK or sentence-case literals in JSX text.
...
- [ ] **Micro-interactions** — purposeful transitions, all gated on `reduced_motion`: ~~task-complete check animation~~ (PR #150 — filled-circle pop + short dwell before the row leaves; dwell skipped under in-product/OS reduced motion), ~~quadrant drag settle~~ (PR #151 — dropped Matrix card plays a scale-in + accent-ring settle so the eye follows where it landed; `useDragSettleStore` + `matrixCardSettle` keyframe, auto-collapsed under reduced motion), pet reaction on streak/level-up, ~~toast enter/exit easing~~ (already shipped — `ToastStack` translateX+opacity with `--ease-enter`), page/route cross-fade (RouteFallback). Prefer CSS transitions + a shared `motion` token over per-component magic numbers. *Remaining: pet streak/level-up reaction.*
