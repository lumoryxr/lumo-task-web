...
- **2026-07-06 (#234 server-side task search `GET /v1/tasks?q=`)** — Security + Robustness audit of the
  M1 keyword-search endpoint, a **distinct query code path** from the plain task list: it appends its
  own `LIKE … ESCAPE '\'` clause and escapes user-supplied LIKE wildcards (`% _ \`) to literals
  (`routes/tasks.ts`). Two invariants live *only* on this separate SQL builder and had **zero** presence
  in the daily suite (only an in-process `api/tasks.test.ts` wildcard test): **(1) Security/IDOR** — the
  search filter is re-scoped to the caller (`WHERE user_id`); a dropped/loosened scope would let one
  tenant surface another tenant's task **titles/descriptions** via `?q=` (an info-disclosure IDOR the
  CRUD-by-id isolation cases never reach). **(2) Robustness** — a user's literal `%` / `_` is escaped;
  dropping the escape turns `?q=%` into "match everything" (defeating the filter / disclosing the full
  list) and `?q=c_t` into a single-char wildcard. Closed with 4 cases over real HTTP + real file SQLite:
  owner's `?q=<own keyword>` finds the owner's task (search works); the **attacker's** `?q=<owner's rare
  keyword>` → 200 with **zero** results (never the owner's row — the scope has teeth); a literal `%`
  (`?q=%25`) returns only tasks that literally contain `%`, **not** the full list; a literal `_`
  (`?q=c_t`) does **not** match `cat` (`_` is not a single-char wildcard). Mutation-tested with **perfect
  specificity**: dropping the search-path `WHERE user_id` reddens **exactly** the attacker case (AC1),
  dropping the wildcard escape reddens **exactly** the two robustness cases (AC2/AC3), each other set
  stays green. Handler verified already scoped + escaped → **gap in the tests, not the code** (no
  production change). dfx 105 → 109.
