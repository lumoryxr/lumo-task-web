...
- **2026-07-02 (#260 settings AI-provider credential confidentiality)** — Route-surface
  audit found the entire `settings` resource was a **silent hole** in the daily
  real-HTTP + real-file-SQLite regression: it appears in the DFX suite only as *setup*
  for the #249 `/ai/classify` IDOR case, never as a subject. Yet `PATCH /v1/settings`
  is the one authenticated write that persists a user-supplied **secret** — the
  AI-provider API key (`ai_configs_update.key`) — with two protections that no
  integration test pinned: **write-only-at-the-wire** (GET/PATCH expose `hasKey`, never
  the plaintext) and **encrypted-at-rest** (AES-256-GCM `enc:v1:…` via `encryptSecret`).
  The in-process `secrets.security.test.ts` covers the wire masking but **not** at-rest
  encryption, and it runs in-process — never over real HTTP in the daily regression. A
  regression that stored the key in plaintext would pass every existing test while
  silently removing at-rest protection (credential leak on any DB/file compromise) —
  provable **only** by inspecting the real SQLite `settings.ai_configs` column. Also
  unpinned: the provider `baseUrl` **SSRF scheme allowlist** (the server later fetches
  it). Closed with 3 cases over real HTTP + real SQLite: key write-only at the wire
  (plaintext absent from PATCH echo + GET); key encrypted at rest (stored column is
  `enc:v1:`, plaintext absent, `decryptSecret` round-trips — teeth); `file:` baseUrl →
  400 `INVALID_BASE_URL` (scheme check precedes the desktop-mode `allowPrivate`
  early-return, so it holds in the local-mode env) + an https positive control. Mutation-
  tested: dropping `encryptSecret` reddens **exactly** the at-rest case (81/82); skipping
  the SSRF guard reddens **exactly** the scheme case (81/82). Handler/contract verified
  already correct — **gap in the tests, not the code** (no production change). dfx 82 ✓.
