...
- [~] **README polish pass** — badges (CI, Release, Demo, License, AI-Coded), demo link, and the "100% AI-coded" note are **already live** in `README.md`. Gap found + fixed this run: the `MIT` badge and "MIT © lumoryxr" footer referenced a **`LICENSE` file that did not exist** (badge 404, no GitHub license detection) → added the canonical MIT `LICENSE` (holder/license already declared by the project, so no new decision). *(2026-08-16)*
